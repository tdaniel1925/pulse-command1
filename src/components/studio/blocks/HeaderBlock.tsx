...
export function HeaderBlock({ content }: { content: KitContent; theme: ThemeProps }) {
  return (
    <header style={{ position: 'sticky', top: 0, zIndex: 50, background: 'var(--surface,#fff)', borderBottom: 'var(--border-w,1px) solid var(--border,#EADFD2)' }}>
      <div className="sx-reveal" style={{ maxWidth: 1180, margin: '0 auto', padding: '16px 32px', display: 'flex', alignItems: 'center', gap: 28 }}>
        ...
        <nav style={{ display: 'flex', gap: 26, marginLeft: 8, fontFamily: "var(--font-body,'Manrope')", fontWeight: 600, fontSize: 15 }}>
          <a href="#" className="sx-link" style={{ color: 'var(--fg)', textDecoration: 'none', opacity: 0.85 }}>Product</a>
          <a href="#" className="sx-link" style={{ color: 'var(--fg)', textDecoration: 'none', opacity: 0.85 }}>Features</a>
          <a href="#" className="sx-link" style={{ color: 'var(--fg)', textDecoration: 'none', opacity: 0.85 }}>Pricing</a>
          <a href="#" className="sx-link" style={{ color: 'var(--fg)', textDecoration: 'none', opacity: 0.85 }}>Company</a>
        </nav>
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 14 }}>
          <a href="#" className="sx-link" style={{ color: 'var(--fg)', textDecoration: 'none', fontFamily: "var(--font-body,'Manrope')", fontWeight: 600, fontSize: 15, opacity: 0.85 }}>Sign in</a>
          <a href="#" className="sx-btn" style={{ display: 'inline-flex', alignItems: 'center', padding: '11px 20px', background: 'var(--btn-bg,#E0603A)', color: 'var(--btn-fg,#fff)', border: '2px solid var(--btn-border,transparent)', borderRadius: 'var(--btn-radius,12px)', boxShadow: 'var(--btn-shadow,none)', fontFamily: "var(--font-body,'Manrope')", fontWeight: 700, fontSize: 15, textDecoration: 'none', transition: 'transform .15s ease' }}>Get started</a>
        </div>
      </div>
    </header>
  )
