        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4" onClick={() => !loading && setOpen(false)}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 space-y-4" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => !loading && setOpen(false)} className="text-neutral-400 hover:text-neutral-700">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div>
              <label className="block text-xs font-semibold text-neutral-500 mb-1.5">
                Schedule for <span className="font-normal text-neutral-400">(optional — defaults to a draft)</span>
              </label>
              <input
                type="datetime-local"
                value={date}
